...
@section('content')
    <div class="min-h-screen flex items-center justify-center bg-cream py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-md w-full space-y-8 bg-white p-8 rounded-2xl shadow-lg">
            <div>
                <h2 class="mt-2 text-center text-3xl font-extrabold text-coffee">Sign in to your account</h2>
                <p class="mt-2 text-center text-coffee/80">Enter your credentials to continue</p>
            </div>
            <form class="mt-8 space-y-6" method="POST" action="{{ route('login') }}">
                ...
                <div class="rounded-md shadow-sm -space-y-px">
                    <div class="mb-4">
                        <label for="email" class="block text-coffee font-semibold">Email</label>
                ...
                <div class="flex items-center justify-between">
                    <label for="remember_me" class="inline-flex items-center">
                        <input id="remember_me" type="checkbox"
                            class="rounded border-coffee/40 text-coffee shadow-sm focus:ring-coffee" name="remember">
                        <span class="ms-2 text-sm text-coffee">Remember me</span>
                    </label>

                    @if (Route::has('password.request'))
                        <a href="{{ route('password.request') }}" class="text-sm text-coffee hover:underline">Forgot your
                            password?</a>
                    @endif
                </div>
                ...
                <div class="text-center">
                    <a href="{{ route('register') }}" class="text-sm text-coffee hover:underline">Don't have an account?
                        Sign up</a>
                </div>
            </form>
        </div>
